...
DEVICE_TYPE = os.environ.get("CPIP_DEVICE", "teapot")
...
usage() {
cat << 'HELP'
cpip — Command-line client for HTCPCP/HTCPCP-TEA (RFC 2324 + RFC 7168)

Usage:
  cpip status                          Show server status
  cpip brew [tea|coffee] [additions]   Start brewing
  cpip pour [tea|coffee] [additions]   Alias for brew
...
[Service]
Type=simple
Environment=CPIP_DEVICE=teapot
...
if systemctl is-active --quiet cpip.service; then
    ...
    fail "Service failed to start — check: journalctl -u cpip"
fi

# ── Summary ──────────────────────────────────────────────────────────
DEVICE=$(grep CPIP_DEVICE /etc/systemd/system/cpip.service | head -1 | cut -d= -f3)
echo ""
echo -e "${GREEN}══════════════════════════════════════════════════════════${NC}"
...
echo "    cpip status              # GET / — server status"
echo "    cpip brew tea            # BREW /tea — start tea"
echo "    cpip pour tea milk;variety=whole  # BREW with additions"
echo "    cpip brew coffee         # BREW /coffee — start coffee"
echo "    cpip when                # WHEN — stop pouring"
echo "    cpip info                # PROPFIND — pot metadata"
echo "    cpip 418                 # Trigger 418 (coffee on teapot)"
echo "    cpip 418-alcohol         # Trigger 418 (alcohol on teapot)"
echo "    cpip additions           # List supported additions"
echo ""
echo -e "${CYAN}  Service Commands:${NC}"
echo "    systemctl status cpip"
echo "    systemctl stop cpip"
echo "    systemctl restart cpip"
echo "    journalctl -u cpip -f"
echo ""
echo -e "${CYAN}  Change device type:${NC}"
echo "    Edit Environment=CPIP_DEVICE= in /etc/systemd/system/cpip.service"
echo "    Then: systemctl daemon-reload && systemctl restart cpip"
